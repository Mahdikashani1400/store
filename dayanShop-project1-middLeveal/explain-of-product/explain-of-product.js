import { $, allProducts, displayOn, displayNone, preLoader, menu, search, load, showProductInfo, virgolPriceOff, virgolPriceOn } from '../functions/functions.js'
...
function executeProductInfo(data) {
    ...
    kindOfSize.innerHTML = 'سایز' + " " + data.title.split(' ').slice(0, -1).join(' ') + " " + ":"

    try {
        displayOn(containProductSize)

        data.sizes.forEach((size) => {
            containProductSize.insertAdjacentHTML('beforeend', `
    ...
    for (let i = 1; i <= 2; i++) {
        ...
        if (i === 1) {
            containSlider.insertAdjacentHTML('beforeend', `  <div class="mini-box-image flex-center"><img src="../images/small${i}-${data.image.split('/')[2]}" alt="1" class="mini-image-product border-mini-image" onclick="selectBorder(event)"></div>`)
            bigImage.setAttribute('src', `../images/small${i}-${data.image.split('/')[2]}`)
        } else {


            containSlider.insertAdjacentHTML('beforeend', `  <div class="mini-box-image flex-center"><img src="../images/small${i}-${data.image.split('/')[2]}" alt="1" class="mini-image-product" onclick="selectBorder(event)"></div>`)
...
function setSize(id) {
    ...
    try {
        let sizeTarget = $.getElementById(id)
            // event.path.filter(function(element) {
            //     if (element.classList.contains('contain-number-size')) {
...
function sameProducts(nameOfProduct) {
    // containProducts.style.width = `${100+(20*(count-5))}%`
...
function runLeftslide() {
    ...
    if (
        containProducts.clientWidth - containProducts.scrollWidth == containProducts.scrollLeft
    ) {
...
function runRightslide() {
    ...
    if (
        ...
        containProducts.scrollLeft = containProducts.clientWidth - containProducts.scrollWidth
    }
...
function addTobasket() {
    try {
        ...
        if (selectedSize) {
            location.href = location.pathname.replace(new RegExp('explain-of-product', 'g'), 'user-basket') +
                '?id=' + productTarget + ',' + selectedSize
        }
    } catch {
        ...
        if (particularSize.innerHTML === '-----') {
            ...
            displayOn(sizeError)
            setTimeout(() => { displayNone(sizeError) }, 1500)
        }
    }
